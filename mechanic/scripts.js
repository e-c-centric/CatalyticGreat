document.addEventListener('DOMContentLoaded', function () {
    ...
    // Recent vehicles
    fetch('../fix.php/recent_vehicles.php')
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn btn-outline btn-sm" onclick="loadVehicle('${vehicle.vin}')">View</button>
                    </td>
                </tr>
            `).join('');
            }
    ...
    if (vehicleInfoContainer) {
        ...
        if (selectedVin) {
            fetch(`../fix.php/vehicle_data.php?vin=${encodeURIComponent(selectedVin)}`)
                .then(response => response.json())
                .then(batches => {
